...
app
  ...
  .get('/daily/comp', async (req, res) => {
    res.redirect('/daily');
  })

  .post("/api/excel_output", async (req, res) => {
    const r = req.body;
    ...
    while(n != r.length) {
      ...
      if(edtime_arr.length === 2 ) {
        console.log(sttime_arr);
        console.log(edtime_arr);
  ...
  where wf.is_deleted = 0  and wf.user_id = ${userId};`;
    ...
    res.render("workflow/sendls.ejs", {
      errors: [],
      send_workflows: send_workflows
    });
  })
  ...
  where wf.id = ${workflow_id} and wf.is_deleted = 0;`;
    ...
    res.render("workflow/send_confirm.ejs", {
      errors: [],
      user: user,
      workflow: workflow,
      status: status_parr
